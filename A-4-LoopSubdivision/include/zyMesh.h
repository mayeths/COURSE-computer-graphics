...
public:
    ...
    // 假定为三角形，求半边对面角的角度（返回弧度制）
    float calcOppoAngle(const HalfedgeHandle & _heh) {
        assert(_heh.next_halfedge_id != IndexNotValid);
        assert(_heh.prev_halfedge_id != IndexNotValid);
        const HalfedgeHandle & _next_heh = halfedgeList[_heh.next_halfedge_id];
        const HalfedgeHandle & _prev_heh = halfedgeList[_heh.prev_halfedge_id];
        assert(_next_heh.to_vertex_id == _prev_heh.from_vertex_id);
        size_t vOppoId = _next_heh.to_vertex_id;
        vec3 vOppo_vFrom = vertexData[_heh.from_vertex_id].Position - vertexData[vOppoId].Position;
        vec3 vOppo_vTo   = vertexData[_heh.to_vertex_id].Position   - vertexData[vOppoId].Position;
        return acos(glm::dot(vOppo_vFrom, vOppo_vTo));// 返回弧度制
    }
