...
class Snow
{
    static inline const float PARTICLE_TYPE_LAUNCHER = 0.0f;
    static inline const glm::vec3 MAX_VELOC = glm::vec3(0.0, -3.0, 0.0);
    static inline const glm::vec3 MIN_VELOC = glm::vec3(0.0, -1.0, 0.0);
    static inline const float MAX_LAUNCH = 1.0f * 1000.0f;
    static inline const float MIN_LAUNCH = 0.5f * 1000.0f;
    static inline const float INIT_SIZE = 10.0f;
    static inline const float MAX_SIZE = 10.0f;
    static inline const float MIN_SIZE = 3.0f;
    static inline const int MAX_PARTICLES = 5000;
    static inline const int INIT_PARTICLES = 500;
   ...
   public:
    Snow()
    {
        ...
        const GLchar* varyings[5] = {
            ...
            "Size1"};  //设置TransformFeedback要捕获的输出变量
        log_info("hahha 1.1");
		mUpdateShader = new Shader("./assets/Shaders/Update.vs", "./assets/Shaders/Update.fs", 
						"./assets/Shaders/Update.gs", varyings, 5);
        // mUpdateShader = new Shader(
        //     "./assets/Snow/Update.vs", "./assets/Snow/Update.fs",
        //     "./assets/Snow/Update.gs", varyings, 5
        // );
        log_info("hahha 1.2");
		...
		mRenderShader = new Shader("./assets/Shaders/Render.vs", 
							"./assets/Shaders/Render.fs");

        // mRenderShader = new Shader("./assets/Snow/Render.vs", "./assets/Snow/Render.fs");
        //设置随机纹理
        log_info("hahha 3.0");
        ...
        // mSparkTexture.loadTexture("./assets/Snow/snowstorm.bmp");
        log_info("hahha 5.0");
        mRenderShader->use();
        log_info("hahha 6.0");
   ...
   private:
    void UpdateParticles(float frametimeMills, float floorY)
    {
        mUpdateShader->use();
        ...
        glBindTransformFeedback(
            GL_TRANSFORM_FEEDBACK, mTransformFeedbacks[mCurTransformFeedbackIndex]
        );
        ...
        glVertexAttribPointer(
            0, 1, GL_FLOAT, GL_FALSE, sizeof(SnowParticle), (void*)offsetof(SnowParticle, type)
        );
        glVertexAttribPointer(
            1, 3, GL_FLOAT, GL_FALSE, sizeof(SnowParticle), (void*)offsetof(SnowParticle, position)
        );
        glVertexAttribPointer(
            2, 3, GL_FLOAT, GL_FALSE, sizeof(SnowParticle), (void*)offsetof(SnowParticle, velocity)
        );
        glVertexAttribPointer(
            3, 1, GL_FLOAT, GL_FALSE, sizeof(SnowParticle),
            (void*)offsetof(SnowParticle, lifetimeMills)
        );
        glVertexAttribPointer(
            4, 1, GL_FLOAT, GL_FALSE, sizeof(SnowParticle), (void*)offsetof(SnowParticle, size)
        );
    ...
    void RenderParticles(glm::mat4& worldMatrix, glm::mat4& viewMatrix, glm::mat4& projectMatrix)
    {
        ...
        glVertexAttribPointer(
            0, 3, GL_FLOAT, GL_FALSE, sizeof(SnowParticle), (void*)offsetof(SnowParticle, position)
        );
        glVertexAttribPointer(
            1, 1, GL_FLOAT, GL_FALSE, sizeof(SnowParticle), (void*)offsetof(SnowParticle, size)
        );
